        ...
        <div key={i} style={{ border: `1px solid ${open === i ? C.gold : C.border}`, borderRadius: 12, overflow: "hidden", transition: "border .25s" }}>
          ...
          >
            ...
            <motion.span
              ...
              style={{ flexShrink: 0, fontSize: 20, color: C.gold, fontWeight: 300, lineHeight: 1 }}
            >
              +
            </motion.span>
          </button>
          <AnimatePresence>
            {open === i && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.28 }}>
                <div style={{ padding: "0 22px 18px", fontSize: 14.5, color: C.slate, lineHeight: 1.75, borderTop: `1px solid ${C.border}` }}>
                  {item.a}
                </div>
              </motion.div>
            )}
