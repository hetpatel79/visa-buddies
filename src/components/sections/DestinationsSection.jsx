...
export default function DestinationsSection({ go, setForm }) {
  return (
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <motion.div variants={containerV} initial="hidden" whileInView="show" viewport={{ once: true }}
          ...
          {DESTINATIONS.map((d, i) => (
            <motion.div key={d.n} variants={fadeUp(i * 0.06)} whileHover={{ y: -6, boxShadow: `0 16px 40px ${C.gold}22`, borderColor: C.gold }}
              onClick={() => { const cn = d.formName || d.n; setForm((f) => ({ ...f, countries: f.countries.includes(cn) ? f.countries : [...f.countries, cn] })); go("assessment"); }}
              style={{ background: C.white, borderRadius: 18, padding: "28px 16px", border: `1px solid ${C.border}`, textAlign: "center", cursor: "pointer", transition: "all .25s" }}>
              ...
              <motion.div whileHover={{ background: C.gold, color: C.navy }}
                style={{ display: "inline-block", padding: "8px 12px", borderRadius: 6, border: `1px solid ${C.gold}`, color: C.gold, fontSize: 11, fontWeight: 700, transition: "all .2s" }}>
