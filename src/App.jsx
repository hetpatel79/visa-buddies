...
function Marquee({ items, speed = 30, reverse = false }) {
  ...
  return (
    <div style={{ overflow: "hidden", display: "flex", width: "100%" }}>
      <motion.div style={{ display: "flex", gap: 0, x: xPct, width: "200%" }}>
        {doubled.map((item, i) => (
          <div key={i} style={{ flexShrink: 0, width: `${100 / doubled.length}%`, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "12px 20px", borderRight: `1px solid ${C.gold}22`, whiteSpace: "nowrap", fontSize: 13, fontWeight: 600, color: C.gold, letterSpacing: ".3px" }}>
            {item}
          </div>
        ))}
      </motion.div>
  ...
  return (
    <div style={{ ...BODY, background: C.white, color: C.text, overflowX: "hidden" }}>
      ...
      <div style={{ background: C.navy, padding: "24px", overflow: "hidden" }}>
        <Marquee items={TRUST.map((t) => `✦ ${t}`)} speed={20} reverse />
      </div>
